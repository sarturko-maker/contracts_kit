---
name: sort
description: Matches every document to an ERP account, writes the entity map, then runs sort.py to build the account folders. Usage /sort [--force].
...
6. List the names that need the user's decision (no row, or confidence `not sure`). Say: edit
   `inputs/entity-map.csv`, set `decided_by` = `user`, run `/sort` again; nothing is asked twice.
7. Say what to run next: `/judge all`.
