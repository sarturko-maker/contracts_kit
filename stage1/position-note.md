...
all longer prose stays here in full, with a link in the overview. Plain words, no recitals;
distinguish established facts, reported extraction and unresolved judgment. Use native sort
cards or, on the table route, imported cells and logged source findings; corrections also apply.
...
governs part of the trade; what is missing. If governing candidates remain unsure, say no
governing agreement is confirmed, name them and say what confirmation is missing. Do not infer
that no contract governs from an empty folder. Describe trading practice only if evidenced.
An invented example:
...
<!-- judge prose: numbered, naming the affected document numbers. One request per underlying
decision/evidence gap, prioritised by materiality. Do not repeat answered questions or convert
every unreviewed extraction cell into a business question. -->
